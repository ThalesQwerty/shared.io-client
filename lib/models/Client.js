...
export class Client extends EventEmitter {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.#ws.onmessage = (event) => {
                try {
                    ...
                    if (output.threadId) {
                        const thread = this.#pendingResponses[output.threadId];
                        thread(output);
                    }
                            ...
                            case "create": {
                                ...
                                if (oldEntity) {
                                    if (oldEntity.owned) {
                                        ...
                                        oldEntity.emit("create", { entity: oldEntity });
                                ...
                                const newEntity = new Entity(channel, output.params.type, output.params.values, output.params.entityId);
                                newEntity.active = true;
                                newEntity.emit("create", { entity: newEntity });                               
    ...
    send(data, onResponse, threadId) {
        if (onResponse && typeof onResponse === "function") {
            this.#pendingResponses[data.threadId] = onResponse;
