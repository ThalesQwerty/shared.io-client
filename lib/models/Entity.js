 ...
 * @extends {TypedEmitter<{
    ...
    "change": (event: { values: Record<string, any>, entity: Entity }) => void
   }>}
 */
export class Entity extends EventEmitter {
    ...
    constructor(channel, values = {}, id = null) {
        ...
        /**
         * @type {{ proxy: Entity, watcher: EventEmitter }}
         */
        const { proxy, watcher } = new WatchedObject(this, Entity.IGNORED_KEYS);
        ...
        if (this.channel) {
            ...
            const changes = { ref: {}, lastUpdate: Date.now() };

            watcher.on("change", event => {
                if (owned) {
                    const hadPreviousChanges = !!Object.keys(changes.ref).length;

                    for (const key in event.values) {
                        if (Entity.isKeyPrivate(key)) continue;
                        changes.ref[key] = event.values[key];
                    }

                    const hasNewChanges = !hadPreviousChanges && Object.keys(changes.ref).length;
